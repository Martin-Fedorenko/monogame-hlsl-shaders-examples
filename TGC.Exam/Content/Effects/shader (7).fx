...
VertexShaderOutput MainVS(in VertexShaderInput input)
{
    ...
    float3 vec = random3(input.Position.xyz);

    float4 meshPos = input.Position;
    meshPos.x += 0.2*sin(Time *50*vec.x);
    meshPos.z += 0.2*sin(Time *50*vec.z);
...
{
    float3 textura = tex2D(textureSampler, input.TextureCoordinate);
    return float4(float3(0.0,textura.g, 0.0), 0.0f);
}
