...
float4 MainPS(VertexShaderOutput input) : COLOR
{
    ...
    float factor1 = step(textureRuido.r, sin(Time*0.5));
    //float4 finalColor = lerp(textureColor,red, factor);
    float factor2 = step(textureRuido.r, sin(Time*0.5+0.05));

    if(factor2){
        textureColor = textureAux;
    }
